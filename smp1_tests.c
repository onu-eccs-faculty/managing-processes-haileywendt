...
int test_sub(int argc, char **argv)
{
       ...
       /* First, check that the subshells were invoked. */
	     fscanf(out, "Shell(pid=%d)1> Shell(pid=%d)1> Shell(pid=%d)1> Shell(pid=%d)1> ", &pids[0], &pids[1], &pids[2], &pids[3]);
	     quit_if(!((pids[0] != pids[1]) && (pids[1] != pids[2]) && (pids[2] != pids[3]) && (pids[0] != pids[2]) && (pids[0] != pids[3]) ));
